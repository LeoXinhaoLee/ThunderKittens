...
void prefill_whole_loop_ker_fp16(
        const int NC,
        __half* __W1, __half* __W2,
        const __half* __XA, const __half* __XB, const __half* __XC,
        __half* __Output
) {
//    H *_W1       = reinterpret_cast<H*>(__W1) + blockIdx.x * W_STRIDE;
//    H *_W2       = reinterpret_cast<H*>(__W2) + blockIdx.x * W_STRIDE;
...
//    const H *_XC       = reinterpret_cast<const H*>(__XC) + blockIdx.x * NC * X_STRIDE;
//    H *_Output = reinterpret_cast<H*>(__Output) + blockIdx.x * NC * X_STRIDE;
    __half *_W1       = __W1 + blockIdx.x * W_STRIDE;
    __half *_W2       = __W2 + blockIdx.x * W_STRIDE;
    const __half *_XA       = __XA + blockIdx.x * NC * X_STRIDE;
    const __half *_XC       = __XC + blockIdx.x * NC * X_STRIDE;
    const __half *_XB       = __XB + blockIdx.x * NC * X_STRIDE;
    __half *_Output = __Output + blockIdx.x * NC * X_STRIDE;
...
//    );
    ...
    __half* W1_data_ptr = reinterpret_cast<H*> (W1.data_ptr<T>());
    __half* W2_data_ptr = reinterpret_cast<H*> (W2.data_ptr<T>());
    const __half* XA_data_ptr = reinterpret_cast<H*> (XA.data_ptr<T>());
    const __half* XB_data_ptr = reinterpret_cast<H*> (XB.data_ptr<T>());
    const __half* XC_data_ptr = reinterpret_cast<H*> (XC.data_ptr<T>());
    __half* Output_data_ptr = reinterpret_cast<H*> (Output.data_ptr<T>());

    const int shmem_size = 0;

    prefill_whole_loop_ker_fp16<<<batch * head, threads, shmem_size, stream>>>(
            NC,
            W1_data_ptr, W2_data_ptr,
            XA_data_ptr, XB_data_ptr, XC_data_ptr,
            Output_data_ptr
